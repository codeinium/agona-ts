...
export function filterPersons(persons: Person[], personType: string, criteria: Partial<Person>): Person[] {
    return persons
        .filter((person : Person): boolean => person.type === personType)
        .filter((person : Person) => {
            const criteriaKeys : (keyof Person)[] = Object.keys(criteria) as (keyof Person)[];
            return criteriaKeys.every((fieldName : keyof Person) : boolean => {
...
export const usersOfAge23 : User[] = filterPersons(persons, 'user', { age: 23 });
export const adminsOfAge23 : Admin[] = filterPersons(persons, 'admin', { age: 23 });
